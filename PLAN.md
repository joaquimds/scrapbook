...
- [x] **Phase 4 — reminders**: daily `croner` cron picks the person whose `greatest(lastContactedAt, last reminder sentAt, createdAt)` is oldest (subject to `REMINDER_COOLDOWN_DAYS`); DMs the user with that person's featured photo (falls back to most-recent tagged photo, then text-only) captioned "Reach out to <name>?". Bot enters `awaitingContactReply`; `yes`/`done` writes `contactLog` and updates `people.lastContactedAt`, `skip`/`no` clears state. Migration `0002_pending_person_ids.ts` adds `pendingPersonIds` jsonb to `ingestion_sessions` so a single session row carries both ingestion and reminder context. `hasUnackedReminder()` makes `runDailyReminder()` idempotent within a 24h ack window.
- [ ] **Phase 5 — polish**: detective-board aesthetic, pan/zoom, edit affordances, person-merge UI.

### Current status

**Phases 1–4 complete.** Server end-to-end smoke-tested via `curl POST /api/scraps` → `GET /api/scraps`. Lint + typecheck (`bun run lint`) clean. Frontend not yet manually verified in the browser — open `http://localhost:5173` and confirm the test scrap appears as a yellow rect node. After that, start **Phase 2 — images**: image messages, kind disambiguation (photo / meme / text-content), multi-image splitting (one scrap per image with shared tags), `sharp` thumbnails, `/media` static serving, image rendering on scrap nodes.
