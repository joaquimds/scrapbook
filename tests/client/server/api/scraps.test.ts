...
describe("GET /api/scraps", () => {
	...
	it("lists scraps in descending creation order", async () => {
		await createScrap(TEST_USER_ID, { body: "first", source: "manual" });
		await createScrap(TEST_USER_ID, { body: "second", source: "manual" });
		const res = await req("GET", "/api/scraps");
	...
	it("paginates with cursor", async () => {
		await createScrap(TEST_USER_ID, { body: "a", source: "manual" });
		await createScrap(TEST_USER_ID, { body: "b", source: "manual" });
		await createScrap(TEST_USER_ID, { body: "c", source: "manual" });
...
describe("POST /api/scraps", () => {
	it("creates a scrap", async () => {
		const res = await req("POST", "/api/scraps", {
			body: { body: "hello world", peopleIds: [] },
		});
	...
	it("creates a scrap with peopleIds", async () => {
		...
		const res = await req("POST", "/api/scraps", {
			body: { body: "tagged", peopleIds: [person.id] },
		});
		expect(res.status).toBe(201);
		const body = await res.json();
	...
	it("returns 400 on invalid body", async () => {
		const res = await req("POST", "/api/scraps", { body: { body: 123 } });
		expect(res.status).toBe(400);
	});
});
